  ...
  const downloadPDF = async () => {
    ...
    try {
      await AuditPDFGenerator.generatePDF({
        ...
        metadata: {
          title: `${submission.checklist_templates?.name || "Staff Report"} - ${organizationData?.organization_name || "Organization"}`,
          author: submission.profiles?.full_name || submission.profiles?.email || "Staff Member",
          subject: "Compliance Audit Report - Industry Standard Format",
          keywords: "compliance, audit, report, staff, checklist, " + (submission.checklist_templates?.name || ""),
